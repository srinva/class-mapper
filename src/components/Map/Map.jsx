...
function getMap(origin, destination) {
  ...
  var transit = 0;
  fetch(
    `https://maps.googleapis.com/maps/api/directions/json?key=${API_KEY}&origin=${originMod}&destination=${destinationMod}&mode=bicycling`,
    { mode: "no-cors" }
  )
    ...
    .then((data) => {
      ...
      fetch(
        `https://maps.googleapis.com/maps/api/directions/json?key=${API_KEY}&origin=${originMod}&destination=${destinationMod}&mode=walking`,
        { mode: "no-cors" }
      )
        .then((response) => response.json())
        .then((data) => {
          ...
          fetch(
            `https://maps.googleapis.com/maps/api/directions/json?key=${API_KEY}&origin=${originMod}&destination=${destinationMod}&mode=transit`,
            { mode: "no-cors" }
          )
            .then((response) => response.json())
            .then((data) => {
